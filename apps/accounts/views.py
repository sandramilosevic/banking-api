            ...
            return Response(
                {
                    ...
                    f"Please try again {settings.LOCKOUT_DURATION.total_seconds() / 60} minutes"
                },
        ...
        try:
            ...
            if user:
                ...
                if failed_attempts >= settings.LOGIN_ATTEMPTS:
                    return Response(
                        {
                            "error": f"You have exceeded the maximum number for login attempts."
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        if refresh_res.status_code == status.HTTP_200_OK:
            ...
            if access_token and refresh_token:
                set_auth_cookies(
                    response_res, access_token=access_token, refresh_token=refresh_token
                )
...
class OTPVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_locked_out:
            return Response(
                {
                    "error": f"Account is locked due to mulitple failed login attempts."
